...
def plot_trade_correlation_analysis(continuous_df, trades_df):
    ...
    scenario_stats = trades_df.groupby('Scenario').agg({
        'Trade_Return_Pct': ['count', 'mean', lambda x: (x > 0).sum()],
        'TP_Hit': 'sum'
    }).round(2)
...
SCENARIO WIN RATES:
Bull: {scenario_stats.loc['bull', ('Trade_Return_Pct', '<lambda>')] if 'bull' in scenario_stats.index else 0}/{scenario_stats.loc['bull', ('Trade_Return_Pct', 'count')] if 'bull' in scenario_stats.index else 0}
Sideways: {scenario_stats.loc['sideways', ('Trade_Return_Pct', '<lambda>')] if 'sideways' in scenario_stats.index else 0}/{scenario_stats.loc['sideways', ('Trade_Return_Pct', 'count')] if 'sideways' in scenario_stats.index else 0}  
Bear: {scenario_stats.loc['bear', ('Trade_Return_Pct', '<lambda>')] if 'bear' in scenario_stats.index else 0}/{scenario_stats.loc['bear', ('Trade_Return_Pct', 'count')] if 'bear' in scenario_stats.index else 0}
